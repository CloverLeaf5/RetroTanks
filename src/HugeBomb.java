...
public class HugeBomb extends BasicWeapon {

	private final int BOMB_SIZE = 10;
	private final int DAMAGE_FACTOR = 30;
	private int explosionCounter = 0;
	private int ticksSinceCollision = 0;
	private Color color = Color.WHITE;

	public HugeBomb(WeaponType type, float x, float y, float velX, float velY, float angleR, int tagNumber) {
		super(type, x, y, velX, velY, angleR, tagNumber);
		this.trueX = x;
		this.collided = false;
		this.explosionComplete = false;
		explosionDiameter = 150;
	}
